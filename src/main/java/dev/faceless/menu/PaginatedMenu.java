...
import dev.faceless.util.ItemCreator;
import dev.faceless.util.MenuUtil;
import dev.faceless.util.TextContext;
...
public abstract class PaginatedMenu {
    ...
    public static ItemStack prevPage() {
        return ItemCreator.get(Material.ARROW)
                ...
                .setPDC(PluginUtils.getPlugin(), "prev-page", PersistentDataType.STRING, "").build();
    }

    public static ItemStack nextPage() {
        return ItemCreator.get(Material.ARROW)
                .setName(TextContext.get().colored(TextContext.GREEN, "Next Page").build(false))
                .setPDC(PluginUtils.getPlugin(), "next-page", PersistentDataType.STRING, "").build();
    }

    public static ItemStack getFiller() {
        return ItemCreator.get(ItemCreator.createNameless(Material.GRAY_STAINED_GLASS_PANE))
                .setPDC(PluginUtils.getPlugin(), "filler-glass", PersistentDataType.STRING, "").build();
    }
    ...
    public ItemStack pageCounter(Player player) {
        return ItemCreator.get(Material.OAK_SIGN)
                ...
                .setPDC(PluginUtils.getPlugin(), "page-counter", PersistentDataType.STRING, "").build();
    }

    public boolean isPageCounter(ItemStack item) {
        return item.getItemMeta().getPersistentDataContainer().has(new NamespacedKey(PluginUtils.getPlugin(), "page-counter"));
    }
